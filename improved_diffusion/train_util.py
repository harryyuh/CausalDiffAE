...
class TrainLoop:
    def __init__(
        ...
        *,
        ...
        if self.resume_step:
            ...
            self.ema_params = [
                copy.deepcopy(self.master_params) for _ in range(len(self.ema_rate)) # COPY OF PARAMETERS
            ]
    ...
    def forward_backward(self, batch, cond):
        zero_grad(self.model_params) # ZERO OUT PARAMS FOR UNET
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(dist_util.dev()) # LOAD MICROBATCH (SUBSAMPLING THE FULL BATCH) GET SOMETHING LIKE (2, 3, 64, 64)
            micro_cond = {
            ...
            # SAMPLE TIMESTEPS FOR EACH IMAGE IN OUR BATCH
            t, weights = self.schedule_sampler.sample(micro.shape[0], dist_util.dev())

            # COMPUTE THE LOSS (THIS IS JUST A WRAPPER SO THAT WE CAN JUST USE COMPUTE_LOSSES() ANYWHERE ELSE WITHOUT REPEATING PARAMETERS
            compute_losses = functools.partial(
                ...
                rep_cond=self.rep_cond,
                causal_modeling=self.causal_modeling
            )
