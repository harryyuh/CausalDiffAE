    ...
    # COUNTERFACTUAL GENERATION
        ...
        while len(all_images) * args.batch_size < args.num_samples:
            ...
            if "morphomnist" in args.data_dir:
                ...
                # thickness = 0.5 + gamma_dist.sample()
                # thickness = np.random.uniform(1., 5.)
                ...
                mu[:, 256:512] = th.ones((args.batch_size, 256)) * t_norm            
                ...
                # mu[:, :256] = th.ones((args.batch_size, 256)) * 0.4
                ...
                # z_post[:, 256:] = th.ones((args.batch_size, 256)) * 0.9
                ...
                # angle_norm = (angle_int - pend_scale[0][0]) / (pend_scale[0][1] - 0)
                angle_norm = (angle_int - pend_scale[3][0]) / (pend_scale[3][1] - 0)
                labels = cond['c'].clone()
                # print(labels[0])
                labels[:, 3] = th.tensor(angle_int)
                
                # print(labels[:, 0].shape)
                # exit(0)
                
                # GENERATE THE GROUND-TRUTH BASED ON CONDITIONED VALUES
                X_real, v = pd.generate(labels[:, 0], labels[:, 1], shadow_pos=angle_int)
                # real_angle.append(th.tensor(X_real))
                ...
                mu[:, 48:64] = th.ones((args.batch_size, 16)) * angle_norm # 30
                # mu[:, 16:32] = th.ones((args.batch_size, 16)) * angle_norm
                ...
                # angle_norm = (angle_int - pend_scale[3][0]) / (pend_scale[3][1] - 0)
                # # angle_norm = (angle_int - pend_scale[1][0]) / (pend_scale[1][1] - 0)
                # labels = cond['c'].clone()
                # # print(labels[0])
                # labels[:, 3] = th.tensor(angle_int)
                ...
                # # mu[:, :16] = th.ones((args.batch_size, 16)) * 0.67 # 30
                # mu[:, 96:128] = th.ones((args.batch_size, 32)) * angle_norm
                ...
                # gathered_samples = [th.zeros_like(sample) for _ in range(dist.get_world_size())]
                # dist.all_gather(gathered_samples, sample)  # gather not supported with NCCL
                # all_images_angle.extend([sample.cpu() for sample in gathered_samples])


                # save_image(batch[:16], '../results/pendulum/diffae_aligned/cfid/original.png')
                # save_image(th.tensor(all_images_angle[0][:16], dtype=th.float32), '../results/pendulum/diffae_aligned/cfid/generated_angle.png')
                # save_image(real_angle[0][:16], '../results/pendulum/diffae_aligned/cfid/true_angle.png')

                # exit(0)
                
                
                # # INTERVENTIONS ON EACH PENDULUM VARIABLE
                # noise = th.randn_like(batch).to(dist_util.dev())
                # t = th.ones((batch.shape[0]), dtype=th.int64) * 249
                # t = t.to(dist_util.dev())
                ...
                # mu, var = model.rep_emb.encode(batch.to(dist_util.dev()))
                # #var = th.ones(mu.shape).to(mu.device) * 0.001
                # mu[:, :32] = th.ones((args.batch_size, 32)) * 0.67
                ...
                # mu, var = model.rep_emb.encode(batch.to(dist_util.dev()))
                # #var = th.ones(mu.shape).to(mu.device) * 0.001
                # mu[:, 32:64] = th.ones((args.batch_size, 32)) * -0.32
                ...
                # mu, var = model.rep_emb.encode(batch.to(dist_util.dev()))
                # var = th.ones(mu.shape).to(mu.device) * 0.001
                # mu[:, 64:96] = th.ones((args.batch_size, 32)) * 0.7
                ...
                # mu, var = model.rep_emb.encode(batch.to(dist_util.dev()))
                # var = th.ones(mu.shape).to(mu.device) * 0.0
                # mu[:, 96:128] = th.ones((args.batch_size, 32)) * 0.8
                ...
                mu, var = model.rep_emb.encode(batch.to(dist_util.dev()))
                #var = th.ones(mu.shape).to(mu.device) * 0.001
                mu[:, :128] = th.ones((args.batch_size, 128)) * 0.3
                ...
                mu, var = model.rep_emb.encode(batch.to(dist_util.dev()))
                #var = th.ones(mu.shape).to(mu.device) * 0.001
                mu[:, 128:256] = th.ones((args.batch_size, 128)) * 0.9
                ...
                mu, var = model.rep_emb.encode(batch.to(dist_util.dev()))
                var = th.ones(mu.shape).to(mu.device) * 0.001
                mu[:, 256:384] = th.ones((args.batch_size, 128)) * 0.9
                ...
                mu, var = model.rep_emb.encode(batch.to(dist_util.dev()))
                var = th.ones(mu.shape).to(mu.device) * 0.0
                mu[:, 384:512] = th.ones((args.batch_size, 128)) * 0.9
        ...
        if generate_interventions:
            if "morphomnist" in args.data_dir:
                ...
                save_image(temp, f'../results/morphomnist/causaldiffae_masked/intervene_thickness_w={w}.png')
                ...
                save_image(temp, f'../results/morphomnist/causaldiffae_masked/intervene_intensity_w={w}.png')
            elif "pendulum" in args.data_dir:
                save_image(batch[0], '../results/pendulum/diffae_aligned/original.png')
                ...
                temp = th.tensor(temp, dtype=th.float32)
                save_image(temp, '../results/pendulum/diffae_aligned/intervene_angle.png')
                ...
                temp = th.tensor(temp, dtype=th.float32)
                save_image(temp, '../results/pendulum/diffae_aligned/intervene_light.png')
                ...
                temp = th.tensor(temp, dtype=th.float32)
                save_image(temp, '../results/pendulum/diffae_aligned/intervene_shadowlen.png')
                ...
                temp = th.tensor(temp, dtype=th.float32)
                save_image(temp, '../results/pendulum/diffae_aligned/intervene_shadowpos.png')
            
            elif "circuit" in args.data_dir:
                save_image(batch[7], '../results/circuit/diffae/original.png')
                ...
                temp = th.tensor(temp, dtype=th.float32)
                save_image(temp, f'../results/circuit/diffae/intervene_arm.png')
                ...
                temp = th.tensor(temp, dtype=th.float32)
                save_image(temp, f'../results/circuit/diffae/intervene_blue.png')
                ...
                temp = th.tensor(temp, dtype=th.float32)
                save_image(temp, f'../results/circuit/diffae/intervene_green.png')
                ...
                temp = th.tensor(temp, dtype=th.float32)
                save_image(temp, f'../results/circuit/diffae/intervene_red.png')
        else:
            if "morphomnist" in args.data_dir:
